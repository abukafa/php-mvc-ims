...
$pdf->SetFont('Arial','',8);
$outlet = $u['kode'];
$masuk = myquery("SELECT * FROM penerimaan WHERE outlet='$outlet' AND tanggal BETWEEN '$tglawl' AND '$tglahr'");
foreach($masuk as $ma) :
...
$pdf->Cell(2, 0.7, number_format($ma['harga'], 0, '.', ','), 1, 0, 'R');
$pdf->Cell(2, 0.7, number_format($ma['jumlah'], 0, '.', ','), 1, 0, 'R');
// $pdf->Cell(2, 0.7, $ma['bayar'], 1, 0, 'R');
$pdf->Cell(7, 0.7, $ma['note'], 1, 1, 'C');
$no++;
endforeach;

$pdf->SetFont('Arial','B',9);
$pdf->Cell(18.5, 0.8, 'Total', 1, 0, 'C');
$total = myquery("SELECT SUM(jumlah) as total FROM penerimaan WHERE outlet='$outlet' AND tanggal BETWEEN '$tglawl' AND '$tglahr'");
